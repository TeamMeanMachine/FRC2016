...
//				System.out.println("Vision loop running");
				...
				if(!camera.read(rawImage)) {
					...
					camera.open(aCam.getVideoURL());
					continue;
				}
				
				// Filter image
				Core.inRange(rawImage, new Scalar(0, 75, 0), new Scalar(150, 255, 150), filteredImage);
	...
	//					Imgproc.line(rawImage, new Point(centerX - 50, centerY), new Point(centerX + 50, centerY), new Scalar(0, 0, 255), 2);
				...
				System.out.println("OpenCV Blob Count: " + blobCount);
				...
				if(blobCount > 0) {
					...
					System.out.println("OpenCV pixelOffset: " + pixelOffset);
					System.out.println("OpenCV GyroTarget " + gyroTarget);
				}
	...
	public void start() {
		if(t == null) {
			t = new Thread(this, "VisionThread");
			t.start();
		}
	}
